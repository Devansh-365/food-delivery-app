...
const CreateContainer = () => {
  ...
  const deleteImage = () => {
    ...
    deleteObject(deleteRef).then(() => {
      ...
      setIsLoading(false)
      setFields(true)
      setMsg('Image delted successfully 🙂 ')
      setAlertStatus('success')
      setTimeout(() => {
        setFields(false)
      },4000)
      ...
      <div className='w-[90%] md:w-[75%] border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center'>
        {
          fields && (
            ...
            animate = {{opacity:1}}
            exit = {{opacity:0}} 
            className={`w-full p-2 rounded-lg text-center text-lg text-semibold ${alertStatus === "danger" ? "bg-red-400 text-red-800" : "bg-emerald-400 text-emerald-800" }`}
            >
            {msg}
            </motion.p>
          )
        ...
        <div className='w-full py-2 border-b border-gray-300 flex items-center gap-2'>
          ...
          className='w-full h-full text-lg bg-transparent font-semibold outline-none border-none placeholder:text-gray-4 text-textColor' />
        </div>
        <div className="w-full">
          ...
          <div className='group flex justify-center items-center flex-col border-2 border-gray-300 w-full h-225 md:h-420 cursor-pointer rounded-lg'>
            {isLoading ? <Loader /> : <>
            {!imageAsset ? (
